    ...
    init() {
        ...
        // Attempt 2: Try in-memory storage
            ...
            DispatchQueue.main.async {
                ...
                alert.informativeText = "NoteScribe couldn't access its storage location. Your transcriptions will not be saved between sessions."
        ...
        let transcriptionState = TranscriptionState(modelContext: container.mainContext)
        _transcriptionState = StateObject(wrappedValue: transcriptionState)

        let hotkeyManager = HotkeyManager(transcriptionState: transcriptionState)
        ...
        Task {
            await transcriptionState.resetOnLaunch()
        }
                ...
                .modelContainer(container)
                    .onAppear {
                        ...
                        if containerInitializationFailed {
                            ...
                            alert.informativeText = "NoteScribe cannot initialize its storage system. The app cannot continue.\n\nPlease try reinstalling the app or contact support if the issue persists."
                    ...
                    .onDisappear {
                        // v1.2: No model unloading needed for Parakeet

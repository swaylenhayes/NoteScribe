...
struct AudioCleanupSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Control how NoteScribe handles your transcription data and audio recordings for privacy and storage management.")
                .font(.system(size: 13))
            ...
            if isTranscriptionCleanupEnabled {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Button(action: {
                        Task {
                            await TranscriptionAutoCleanupService.shared.runManualCleanup(modelContext: transcriptionState.modelContext)
                            await MainActor.run {
            ...
            if isAudioCleanupEnabled && !isTranscriptionCleanupEnabled {
                ...
                Button(action: {
                    ...
                    Task {
                        ...
                        // Get cleanup info
                        let info = await AudioCleanupManager.shared.getCleanupInfo(modelContext: transcriptionState.modelContext)
                        ...
                        Button("Delete \(cleanupInfo.fileCount) Files", role: .destructive) {
                            Task {
                                ...
                                let result = await AudioCleanupManager.shared.runCleanupForTranscriptions(
                                    modelContext: transcriptionState.modelContext, 
        ...
        .onChange(of: isTranscriptionCleanupEnabled) { _, newValue in
            if newValue {
                ...
                AudioCleanupManager.shared.startAutomaticCleanup(modelContext: transcriptionState.modelContext)
            }
        }
    }
} 
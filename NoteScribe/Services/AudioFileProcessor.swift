...
class AudioProcessor {
    private let logger = Logger(subsystem: "com.swaylenhayes.apps.notescribe", category: "AudioProcessor")
    ...
    func processAudioToSamples(_ url: URL) async throws -> [Float] {
        ...
        while currentFrame < totalFrames {
            ...
            if sampleRate == AudioFormat.targetSampleRate && channels == AudioFormat.targetChannels {
                let chunkSamples = convertToAudioFormat(inputBuffer)
                ...
                let chunkSamples = convertToAudioFormat(outputBuffer)
                allSamples.append(contentsOf: chunkSamples)
            }
    ...
    private func convertToAudioFormat(_ buffer: AVAudioPCMBuffer) -> [Float] {

...
class AudioDeviceManager: ObservableObject {
    ...
    func selectDevice(id: AudioDeviceID) {
        logger.info("Audio input selection disabled; using system default.")
        fallbackToDefaultDevice()
    }
    ...
    func selectInputMode(_ mode: AudioInputMode) {
        logger.info("Audio input mode selection disabled; forcing system default.")
        inputMode = .systemDefault
        selectedDeviceID = nil
        UserDefaults.standard.removeObject(forKey: UserDefaults.Keys.selectedAudioDeviceUID)
        notifyDeviceChange()
    ...
    private func getDeviceProperty<T>(deviceID: AudioDeviceID,
        ...
        let propertyPointer = UnsafeMutableRawPointer.allocate(
        ...
        return propertyPointer.assumingMemoryBound(to: T.self).pointee
    }

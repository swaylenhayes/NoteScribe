...
class AudioTranscriptionManager: ObservableObject {
    ...
    func startProcessing(url: URL, modelContext: ModelContext, transcriptionState: TranscriptionState) {
        ...
        currentTask = Task {
            do {
                guard let currentModel = transcriptionState.currentTranscriptionModel else {
                ...
                // Transcribe using Parakeet service
                ...
                text = try await parakeetTranscriptionService!.transcribe(audioURL: permanentURL, model: currentModel, useVAD: useVAD)
                logger.notice("[perf] File manager transcription total: \(String(format: "%.2f", Date().timeIntervalSince(transcriptionStart)))s (useVAD=\(useVAD))")

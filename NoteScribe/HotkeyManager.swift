...
class HotkeyManager: ObservableObject {
    ...
    private var canProcessHotkeyAction: Bool {
        transcriptionState.recordingState != .transcribing && transcriptionState.recordingState != .enhancing && transcriptionState.recordingState != .busy
    }
    ...
    init(transcriptionState: TranscriptionState) {
        ...
        self.miniRecorderShortcutManager = MiniRecorderShortcutManager(transcriptionState: transcriptionState)

        KeyboardShortcuts.onKeyUp(for: .pasteLastTranscription) { [weak self] in
            guard let self = self else { return }
            Task { @MainActor in
                LastTranscriptionService.pasteLastTranscription(from: self.transcriptionState.modelContext)
            }
        }
        ...
        KeyboardShortcuts.onKeyUp(for: .pasteLastEnhancement) { [weak self] in
            guard let self = self else { return }
            Task { @MainActor in
                LastTranscriptionService.pasteLastEnhancement(from: self.transcriptionState.modelContext)
            }
        }
        ...
        KeyboardShortcuts.onKeyUp(for: .retryLastTranscription) { [weak self] in
            guard let self = self else { return }
            Task { @MainActor in
                LastTranscriptionService.retryLastTranscription(from: self.transcriptionState.modelContext, transcriptionState: self.transcriptionState)
            }
        }
